...
void BrainLog::logStatics()
{
    ...
    // draw center circle
    vector<rerun::Vec2D> circleCenter = {{fd.circleRadius, 0}};
    for (int i = 0; i < 360; i++)
    {
        double r = fd.circleRadius;
        double theta = (i + 1) * M_PI / 180;
        circleCenter.push_back(rerun::Vec2D{r * cos(theta), r * sin(theta)});
    }
    mapLines.push_back(rerun::LineStrip2D(circleCenter));
    ...
    circles.push_back(this->circle(brain->config->fieldDimensions.length/2.0, 0,
                             brain->config->kickRadiusToGoal));
    log("striker/kick_radius_to_goal",
...
void BrainLog::logDrible(string logPath)
{
    vector<Point2D> oppPoints;
    Point2D ballPos = Point2D{brain->data->ball.posToField.x, brain->data->ball.posToField.y};
    vector<u_int32_t> color = {};

    Point2D dribleDir = brain->findDribleDir(oppPoints);
    vector<rerun::LineStrip2D> lines = {};

    for (auto &pos : oppPoints)
    {
        if (pos.norm() > 0)
        {
            lines.push_back(rerun::LineStrip2D({{pos.x, -pos.y}, {ballPos.x, -ballPos.y}}));
            color.push_back(0x00FF00FF);
        }
    }

    lines.push_back(rerun::LineStrip2D({{dribleDir.x, -dribleDir.y},
                                        {ballPos.x, -ballPos.y}}));
    color.push_back(0xFFFF00FF);
