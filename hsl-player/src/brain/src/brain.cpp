...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<bool>("sound.enable", true);
    ...
    if (joy.rt) { // RT 组合键
        {
            ...
            prtDebug("State => 4:  TESTING");
            // playSound("exited");
        }

        //trocar entre os chutes
        if(joy.a){
            tree->setEntry<string>("test_kick_type", "kickinho");
            prtDebug("kickinho");
        }

        else if(joy.b){
            tree->setEntry<string>("test_kick_type", "kick");
            prtDebug("kick");
        }

        else if(joy.x){
            tree->setEntry<string>("test_kick_type", "kickao");
            prtDebug("kickao");
        }

        else if(joy.y){
            tree->setEntry<string>("test_kick_type", "drible");
            prtDebug("drible");
        }
